...
public class DBMain {
	 ...
	 * @value 2.自动建表
	 * @value 3.智能检测字段信息,并自动执行新增、删除、修改字段长度sql
	...
	public static void addVo() {
		List<Class<?>> list = DataBase.classList;
		 list.add(GyDept.class);
		// list.add(PzJeiKou.class);
...
//		list.addAll(getClassList("cn.ms.user.schema"));
	...
	public static void zhuShi() throws Exception {
		...
		map.put("SysUser", VoConvertUtil.voToMap(new SysUser()));
		map.put("SysUserRole", VoConvertUtil.voToList(new SysQuanXian()));
	...
	public static void mysql() {
		...
		// String url =
		// "jdbc:mysql://10.10.4.63:3306/ms?useUnicode=true&characterEncoding=UTF8";
		String userName = "root";
		String password = "Ytx_!rb123";
		DataBase.setDB(driver, url, userName, password);
	}

	public static void mysqlBenDi() {
		String driver = "com.mysql.jdbc.Driver";
		String url = "jdbc:mysql://localhost:3306/mcp32000000";
		String userName = "root";
		String password = "mcpdbtest";
		DataBase.setDB( driver, url, userName, password);
	}
